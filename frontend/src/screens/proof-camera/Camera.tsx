...
const ProofVisionCameraScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  // FIX 2: Use a ref for offline status so all callbacks always read the latest
  // value synchronously — avoids stale closure on first render.
  const offlineRef = useRef(false);
  const [offline, setOffline] = useState(false);

  // FIX 3: Set up NetInfo listener AND fetch initial network state immediately
  // on mount so offlineRef is populated before any habits fetch runs.
  useEffect(() => {
  ...
  const fetchHabits = useCallback(async (query: string) => {
    ...
    try {
      ...
      // FIX 4: Guard against empty/undefined API response — don't overwrite good cache.
      if (!normalized || normalized.length === 0) {
        console.log('[Habits] API returned empty — keeping cache.');
      ...
      // FIX 5: On error, keep cache already in state — don't clear habits.
      console.log('[Habits] fetchHabits error — keeping cache:', err);
      showMessage('Failed to load activities. Please try again.');
    } finally {
      setHabitsLoading(false);
    }
  }, [habitId, selectedHabit]); // FIX 6: No `offline` dependency — uses ref instead.

  // FIX 7: On mount, always load cache first unconditionally, then attempt
  // online fetch. Previously the online check used stale offline state.
  useEffect(() => {
    (async () => {
      ...
      // Step 2: Fetch live data only if online (read ref, not state).
  ...
  // FIX 8: Re-fetch when coming back online so live data replaces cache.
  useEffect(() => {
    if (!offline) {
      fetchHabits(search);
    }
  ...
  // FIX 9: When modal opens, only fetch if online (use ref for synchronous check).
  useEffect(() => {
    if (habitModalVisible && !offlineRef.current) {
      fetchHabits(search);
    }
  }, [habitModalVisible]);
  ...
  const handleSearchHabit = (txt: string) => {
    ...
    // FIX 10: Use offlineRef for synchronous check — never use offline state here.
    if (!offlineRef.current) {
  ...
  const selectedLabel = selectedHabit?.habitName || 'Tap to select an activity';
...
const styles = StyleSheet.create({
  ...
  glowBottom: {
    position: 'absolute', bottom: -140, right: -40, width: 260, height: 260,
  ...
  iconGlass: {
    ...
    shadowColor: '#000', shadowOpacity: 0.25, shadowOffset: { width: 0, height: 8 },
    shadowRadius: 16, elevation: 6,
  },
  title: { color: '#F9FAFB', fontSize: 16, fontWeight: '700' },
  selectedHabitBar: { position: 'absolute', left: 20, right: 20, bottom: 140 },
  selectedHabitButton: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: GLASS_BG, borderRadius: 999,
  ...
  selectedHabitLabel: { color: '#E5E7EB', fontSize: 14, fontWeight: '600' },
  selectedHabitTime: { color: '#9CA3AF', fontSize: 11, marginTop: 2 },
  bottomBar: {
  ...
  shutterOuterGlass: {
    ...
    borderWidth: 3, borderColor: 'rgba(191, 219, 254, 0.8)',
    alignItems: 'center', justifyContent: 'center',
    backgroundColor: 'rgba(15, 23, 42, 0.7)',
    shadowColor: '#60A5FA', shadowOpacity: 0.6,
    shadowOffset: { width: 0, height: 12 }, shadowRadius: 24, elevation: 10,
  },
  shutterInner: { width: 58, height: 58, borderRadius: 29, backgroundColor: '#F9FAFB' },
  modalContainer: {
    flex: 1, justifyContent: 'center', alignItems: 'center',
    backgroundColor: 'rgba(0,0,0,0.0)', paddingHorizontal: 16, marginBottom: 100,
  },
  modalGlassCard: {
    width: '100%', maxHeight: '75%', borderRadius: 20, padding: 14,
  ...
  searchWrapper: {
    ...
    borderWidth: 1, borderColor: 'rgba(148,163,184,0.6)', marginBottom: 10,
  },
  searchBar: { flex: 1, color: '#F9FAFB', fontSize: 14, paddingVertical: 4 },
  modalLoading: { alignItems: 'center', paddingVertical: 16 },
  sectionHeader: { paddingVertical: 6 },
  sectionHeaderText: { color: '#9CA3AF', fontSize: 12, fontWeight: '600', textTransform: 'uppercase' },
  habitItem: { paddingVertical: 8, borderBottomWidth: 0.1, borderBottomColor: "white" },
  habitIconCircle: {
    width: 32, height: 32, borderRadius: 16,
    backgroundColor: 'rgba(55,65,81,0.9)',
    justifyContent: 'center', alignItems: 'center', marginRight: 10,
  },
  habitNameText: { color: '#E5E7EB', fontSize: 14, fontWeight: '500' },
  habitTimeText: { color: '#9CA3AF', fontSize: 11, marginTop: 2 },
  modalClose: {
    marginTop: 10, alignSelf: 'flex-end', paddingHorizontal: 12, paddingVertical: 6,
    borderRadius: 999, borderWidth: 1, borderColor: 'rgba(191, 219, 254, 0.6)',
  },
  modalCloseText: { color: '#E5E7EB', fontSize: 12, fontWeight: '600' },
});
